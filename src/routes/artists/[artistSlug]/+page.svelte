<script lang="ts">
	...
	import { canonicalUrl, jsonLdBreadcrumb, jsonLdMusicGroup, ogMeta } from '$lib/helpers/seo';
	...
	const musicGroupLd = jsonLdMusicGroup({
		...
		albums: data.artist.albums.map((a: { name: string; slug: string }) => ({
			name: a.name,
			slug: a.slug
		}))
	});
